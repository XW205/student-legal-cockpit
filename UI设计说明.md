# 法护启航 · UI 界面设计说明（v2.2 政务 / 公益风格 · 移动端底部导航）
...
- **顶部导航（桌面 ≥961px）**：横向 7 项导航 + 品牌 + “公益演示”标识，导航项复用 `.nav-item`，由 `showPage()` 统一高亮；
- **移动端导航（≤960px）**：隐藏顶部导航栏，改为**底部 Tab 导航**：`首页` / `AI测评` / `AI审查` / `咨询` / `更多`；点「更多」从底部滑出“全部功能”弹层（`.sheet`），含全部 7 页入口与 12348 提示，选中即跳转并自动关闭；适配 `env(safe-area-inset-bottom)`。
...
- **响应式断点**：≤960px：顶栏隐藏、改底部 Tab 导航与“全部功能”弹层，双栏变单栏；≤640px 移动端单栏 + 卡片堆叠 + 地图高度压缩；含 `prefers-reduced-motion` 支持。
...
- [x] 桌面端顶部导航 + 移动端底部 Tab 导航与“全部功能”弹层；网格卡片布局响应式
- [x] 原有全部功能保留：20 题测评、AI 问答（离线 / DeepSeek）、合同上传 / 扫描 / 报告 / 导出、证据清单 / 证据链 / 时间轴、地图点位交互、12348 指引
---

## 3.1 UI 动效规范（专业 · 克制 · 可降级）

在“禁止卡通幼稚 / 过度动画”前提下，为页面加入**克制的可用性动效**，全部满足 `prefers-reduced-motion` 自动降级：

| 动效 | 实现 | 触发 |
|---|---|---|
| 页面切换入场 | `.page.active` 淡入上移 | 导航切换 |
| 滚动显现（交错） | 容器加 `.rv-wrap`，进入视口后子元素按序淡入上移（`rvIn`，间距 80ms） | 首页各卡片区、测评/地图/关于等 15 组容器 |
| 导航下划线生长 | `.topnav .nav-item::after` scaleX | 悬停 / 激活 |
| 主按钮扫光 + 上浮 | `.btn::before` 斜向高光 | 悬停（非 ghost） |
| 测评选项逐条入场 | `.quiz-opt` optIn | 渲染新题目 |
| 链接箭头微位移 | `.kc-link/.mc-go svg` | 悬停 |

实现位置：`styles.css` 末尾「UI 微动效」段 + `index.html` 底部内联脚本（IntersectionObserver，进入视口后为 `.rv-wrap` 添加 `.in`，仅触发一次）。新增动效均为 0.2–0.6s、无循环弹跳；如需增删动画，改动集中在上述两处即可。